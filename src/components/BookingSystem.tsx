...
export default function BookingSystem({ location }: { location: LocationConfig }) {
    ...
    <div className="min-h-screen text-slate-900 dark:text-slate-100 bg-transparent font-sans pb-20 transition-colors duration-500">
      <section className="pt-24 pb-16 px-6 text-center">
        ...
        <p className="text-xl md:text-2xl text-slate-500 dark:text-slate-400 max-w-2xl mx-auto mb-12 font-bold italic tracking-tight">
          {t.subtitle} <br/> <span className="text-primary uppercase tracking-widest text-xs not-italic font-black">Location: {location.name}</span>
        </p>

        <div className="max-w-4xl mx-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[4rem] p-10 md:p-20 shadow-2xl">
          {/* STEP INDICATORS */}
          <div className="flex flex-wrap justify-center gap-4 mb-16">
            {[1, 2, 3, 4].map(s => (
              <div key={s} className={`px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${step === s ? 'bg-primary text-white italic shadow-lg scale-110' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'}`}>
                {t[`step${s}`]}
              </div>
            ))}
          </div>
          ...
          {step === 1 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-12">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {(['general', 'toddler', 'party', 'group'] as BookingType[]).map(bt => (
                  <button 
                    key={bt}
                    onClick={() => setType(bt)}
                  ...
                  >
                    <p className={`text-2xl font-black italic uppercase mb-2 ${type === bt ? 'text-primary' : 'text-slate-900 dark:text-white'}`}>{t[bt]}</p>
              ...
              <div>
                <p className="text-xs font-black uppercase tracking-[0.3em] text-slate-400 mb-8 italic text-center">Select Duration</p>
                <div className="flex flex-wrap justify-center gap-4">
                  {Object.keys(location.pricing.general).map(d => (
                    <button 
                      key={d}
              ...
              <div className="pt-10 flex justify-center">
                 <button onClick={() => setStep(2)} className="bg-primary hover:bg-blue-600 text-white font-black uppercase py-6 px-16 rounded-[2rem] text-xl transition-all shadow-2xl italic tracking-tight">
                   {t.next}
                 </button>
              </div>
            </motion.div>
          ...
          {/* SIMPLIFIED FOR DEMO: Price Summary */}
          {step > 1 && (
             <div className="text-center py-20">
                <p className="text-slate-400 font-black uppercase tracking-widest mb-4 italic">{t.total}</p>
                <h2 className="text-8xl font-black italic text-primary tracking-tighter mb-10">${currentPrice}</h2>
                <div className="flex justify-center gap-6">
                   <button onClick={() => setStep(1)} className="text-slate-400 font-black uppercase italic hover:text-primary transition-colors">{t.prev}</button>
                   <button onClick={() => alert('Booking Redirecting to: ' + location.links.booking)} className="bg-primary text-white px-10 py-5 rounded-2xl font-black uppercase italic shadow-xl">{t.bookNow}</button>
                </div>
             </div>
          )}
        </div>
