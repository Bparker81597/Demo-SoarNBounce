...
export default function WaiverSystem({ location }: { location: LocationConfig }) {
  ...
  return (
    <div className="min-h-screen text-slate-900 dark:text-slate-100 bg-transparent font-sans pb-20 transition-colors duration-500">
      <section className="pt-24 pb-16 px-6 text-center relative">
        ...
        <p className="text-xl md:text-2xl text-slate-500 dark:text-slate-400 max-w-2xl mx-auto mb-10 font-bold italic tracking-tight">
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
           ...
           >
              ...
              <h3 className={`text-2xl font-black uppercase italic mb-2 ${waiverType === 'family' ? 'text-primary' : 'text-slate-900 dark:text-white'}`}>{t.familyTitle}</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400 font-bold italic">{t.familyDesc}</p>
           </button>
           ...
           >
              <span className="text-4xl mb-6 block">👤</span>
              <h3 className={`text-2xl font-black uppercase italic mb-2 ${waiverType === 'single' ? 'text-primary' : 'text-slate-900 dark:text-white'}`}>{t.singleTitle}</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400 font-bold italic">{t.singleDesc}</p>
           </button>
        </div>

        <div className="max-w-xl mx-auto bg-white dark:bg-slate-900 p-12 rounded-[4rem] border border-slate-100 dark:border-slate-800 shadow-2xl">
           <div className="text-left mb-10">
              <label className="block text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 mb-4 ml-4 italic">{t.emailLabel}</label>
              <input 
                type="email" 
                value={email}
